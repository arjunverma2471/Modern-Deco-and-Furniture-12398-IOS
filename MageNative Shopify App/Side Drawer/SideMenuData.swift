              ...
              if let count  = menuItem.menuItems?.count {
                ...
                if count > 0 {
                    self.shopifyFechSubMenu(menu: subMenu) { db in
                        ...
                        let temp = MenuObject(name: translatedStr, children: db, id: menuItem.menuID ?? "", image: "",type: menuItem.menuType ?? "",url: menuItem.menuUrl?.absoluteString ?? "")
      ...
      //  }
        ...
        for items in menu! {
          ...
          if items.menuItems!.count > 0 {
              ...
              let temp = MenuObject(name: translatedStr,children: db, id: items.menuID ?? "", image: "",type: items.menuType ?? "",url: items.menuUrl?.absoluteString ?? "")
