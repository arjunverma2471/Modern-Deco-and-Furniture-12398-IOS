...
//                                        self.mainStack.insertArrangedSubview(self.similarProductView, belowArrangedSubview: self.descriptionView)
    ...
    func fetchKiwiSizeData(checkForSizeChart:Bool) -> URL? {
        ...
        guard let urlRequest = url.getURL() else {return nil}
        print(urlRequest)
...
extension ProductVC {
    ...
    func getArgoidSimilarProducts() {
        let argoidViewModel = ArgoidViewModel()
        let productID = product.id
        self.fetchCustomerDetails { custId in
            argoidViewModel.fetchSimilarProducts(user_id: custId ?? "", number_of_recommendations: 12, include_out_of_stock: false, product_id: productID) {[weak self] result in
                switch result {
                case .success:
                    print("success")
                    if !argoidViewModel.recommendedProducts.isEmpty {
                        if let argoidRecommendationsView = self?.argoidRecommendationsView {
                            self?.mainStack.addArrangedSubview(argoidRecommendationsView)
                            self?.argoidRecommendationsView.delegate = self
                            self?.argoidRecommendationsView.titleLabel.text = "Similar Products".localized
                            self?.argoidRecommendationsView.recommendedProducts = argoidViewModel.recommendedProducts
                            self?.argoidRecommendationsView.heightAnchor.constraint(equalToConstant: 345).isActive = true
                        }
                    }
    ...
    func getArgoidSRecentProducts() {
        let argoidViewModel = ArgoidViewModel()
        let productID = product.id
        self.fetchCustomerDetails { custId in
            argoidViewModel.fetchRecentlyViewedProducts(user_id: custId ?? "", number_of_recommendations: 12, include_out_of_stock: false, product_id: productID) {[weak self] result in
                switch result {
                case .success:
                    print("sucess")
                       if !argoidViewModel.recentlyViewProducts.isEmpty {
                           if let argoidRecentlyViewed = self?.argoidRecentlyViewed {
                               self?.mainStack.addArrangedSubview(argoidRecentlyViewed)
                               self?.argoidRecentlyViewed.delegate = self
                               self?.argoidRecentlyViewed.titleLabel.text = "Recently Viewed Products".localized
                               self?.argoidRecentlyViewed.recommendedProducts = argoidViewModel.recentlyViewProducts
                               self?.argoidRecentlyViewed.heightAnchor.constraint(equalToConstant: 345).isActive = true
                           }
                     }
    ...
    func getArgoidStyledProducts() {
        let argoidViewModel = ArgoidViewModel()
        let productID = product.id
        self.fetchCustomerDetails { custId in
            argoidViewModel.fetchStyledProducts(user_id: custId ?? "", number_of_recommendations: 12, include_out_of_stock: false, product_id: productID) {[weak self] result in
                switch result {
                case .success:
                    print("sucess")
                       if !argoidViewModel.styledProducts.isEmpty {
                           if let argoidRecentlyViewed = self?.argoidStyledProducts {
                               self?.mainStack.addArrangedSubview(argoidRecentlyViewed)
                               self?.argoidStyledProducts.delegate = self
                               self?.argoidStyledProducts.titleLabel.text = "Style It With".localized
                               self?.argoidStyledProducts.recommendedProducts = argoidViewModel.styledProducts
                               self?.argoidStyledProducts.heightAnchor.constraint(equalToConstant: 345).isActive = true
                           }
                     }
