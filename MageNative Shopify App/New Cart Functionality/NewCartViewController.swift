...
class NewCartViewController : BaseViewController {
    ...
    var cartARgoidRecommendations = [ProductViewModel]()
    ...
    func setupTable(){
      ...
      self.tableView.delegate = self
       // self.tableView.reloadData()
        ...
        self.tableView.register(RecommendationsTableCell.self, forCellReuseIdentifier: RecommendationsTableCell.className)
        //
         ...
         //   self.view.stopLoader()
            if let checkout = checkout {
                ...
                if customAppSettings.sharedInstance.argoidRecommendations {
                    self.getArgoidRecommendations(checkout: checkout)
                }
...
//        return 1;
            ...
            return customAppSettings.sharedInstance.argoidRecommendations && self.cartARgoidRecommendations.count > 0 ? 1 : 0
        }
    }
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        else if indexPath.section == 5
      {
            ...
            return customAppSettings.sharedInstance.argoidRecommendations && self.cartARgoidRecommendations.count > 0 ? 345 : 0
        }
    }
  ...
  //          return cell
            ...
            cell.recommendedProducts = self.cartARgoidRecommendations
            return cell
        }
    }
...
extension NewCartViewController{
    ...
    func getArgoidRecommendations(checkout:CartCheckoutViewModel) {
        let argoidViewModel = ArgoidViewModel()
        self.fetchCustomerDetails { custId in
            let ids = checkout.lineItems.map{$0.productId}
            argoidViewModel.fetchCartSimilarProducts(user_id: custId ?? "", number_of_recommendations: 12, product_ids: ids) { [weak self] result in
                switch result {
                case .success:
                    if !argoidViewModel.cartSimilarProducts.isEmpty {
                        self?.cartARgoidRecommendations = argoidViewModel.cartSimilarProducts
                        self?.tableView.reloadData()
                    }
                case .failed(let err):
                    print("Argoid \(err)")
