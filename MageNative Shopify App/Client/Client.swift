...
//      return "50shadesofcloths.myshopify.com"
     return "modern-8220.myshopify.com"//""//"lilystore28.myshopify.com"//
...
//        return "7268"
  ...
  static var apiKey: String{
    if let url = UserDefaults.standard.value(forKey: "apiKey") as? String{
      ...
      return "c712850c62895f4ff5f05f18219fc89a"
//        return "ef4bcbd0c38ababb0754080cdfd25629"
    ...
    static var argoidAuthToken = "YXJnb2lkOkNKaEFkMU1tMDc="
    
    static var argoidStoreName = "magenative-integration"
...
/*  @discardableResult
 ...
 /*   @discardableResult
  ...
  func fetchCompletedPayment(_ id: String, completion: @escaping (PaymentViewModel?,Graph.QueryError?) -> Void) {
    ...
    let retry = Graph.RetryHandler<MobileBuySDK.Storefront.QueryRoot>(endurance: .infinite) { (response, error) -> Bool in
        print("ErorfetchCompletedOrder==",error)
        print("ErorfetchCompletedOrder==",error?.localizedDescription)
    ...
    let task  = self.client.queryGraphWith(query, retryHandler: retry) { response, error in
      error.errorPrint()
        print("ErorfetchCompletedOrderTask==",error.errorPrint())
      let checkout = (response?.node as? MobileBuySDK.Storefront.Checkout)
