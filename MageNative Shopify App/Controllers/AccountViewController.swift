...
class AccountViewController: BaseViewController {
  ...
  override func viewDidLoad() {
    ...
    if customAppSettings.sharedInstance.rewardify && customAppSettings.sharedInstance.flitsIntegration{
      ...
      if customAppSettings.sharedInstance.sealSubscription{
          options.append("My Seal Subscription".localized)
          customImages.append("orderN")
      }
...
//
  ...
  func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    ...
    case 0:
      ...
      }else if title == "My Seal Subscription".localized{
          let vc = SealSubscriptionVC()
          self.navigationController?.pushViewController(vc, animated: true)
