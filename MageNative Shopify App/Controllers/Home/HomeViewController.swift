...
class HomeViewController: BaseViewController {
    ...
    let argoidViewModel = ArgoidViewModel()
    var homeTopDealsArgoid = [ProductViewModel]()
    var homeRecommendationsArgoid = [ProductViewModel]()
    var homeTrendingArgoid = [ProductViewModel]()
    var homeArrivalsArgoid = [ProductViewModel]()
    
    //MARk:- Storista Properties
    var storistaAPI: StoristaAPIHandler? = StoristaAPIHandler()
    ...
    override func viewDidLoad() {
      ...
      /*  if Client.merchantID == "18" {
        ...
        checkForAppUpdate()
        if customAppSettings.sharedInstance.storista{
            storistaAPI?.getWidgetInfo({ [ weak self] result in
                guard let result = result else {return}
                self?.storistaResult = result
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if customAppSettings.sharedInstance.argoidRecommendations {
            self.fetchHomeArgoidData()
        }
        // End--InstaFeeds
        // JS
...
extension HomeViewController:UITableViewDelegate {
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        let argoidCount     = customAppSettings.sharedInstance.argoidRecommendations ? 1 : 0
...
//                    }
        ...
        else if(indexPath.section == (sortOrder?.fields?.count ?? 0) + 2) && customAppSettings.sharedInstance.argoidRecommendations{
            switch indexPath.row {
            case 0 :
                return self.homeTopDealsArgoid.count > 0 ? 345 : 0
            case 1 :
                return self.homeRecommendationsArgoid.count > 0 ? 345 : 0
            case 2 :
                return self.homeTrendingArgoid.count > 0 ? 345 : 0
            default :
                return self.homeArrivalsArgoid.count > 0 ? 345 : 0
            }
        }else if(indexPath.section == (sortOrder?.fields?.count ?? 0) + 2 + argoidCount) && customAppSettings.sharedInstance.storista{
            if storistaAPI?.getLayout() == "slider" {
                return 280
            }else if storistaAPI?.getLayout() == "grid"{
                return storistaCollectionHeight()
            }else if storistaAPI?.storistaFeedWidget?.type == "highlights"{
                return 150
            }else{
                return 300
            }
...
//    }
    ...
    func numberOfSections(in tableView: UITableView) -> Int {
        // Insta Condition
        let argoidCount     = customAppSettings.sharedInstance.argoidRecommendations ? 1 : 0
        let storistaCount   = customAppSettings.sharedInstance.storista ? 1 : 0
        
        if instaFeedCounts>0{
            return (self.sortOrder?.fields?.count ?? 0)+2+argoidCount+storistaCount//manually +3 at bottom cell but if dynamic no need to condition
        }
        else{
            return (self.sortOrder?.fields?.count ?? 0)+2+argoidCount+storistaCount
        }
    }
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        if (section == (self.sortOrder?.fields?.count ?? 0) + 2) && customAppSettings.sharedInstance.argoidRecommendations {
            var count = 0
            count += homeRecommendationsArgoid.count > 0 ? 1 : 0
            count += homeTopDealsArgoid.count > 0 ? 1 : 0
            count += homeTrendingArgoid.count > 0 ? 1 : 0
            count += homeArrivalsArgoid.count > 0 ? 1 : 0
            return count
        }
        
        if (section == (self.sortOrder?.fields?.count ?? 0) + 2 + argoidCount) && customAppSettings.sharedInstance.storista {
            var count = 0
            count += (storistaResult?.data?.first?.stories?.count) ?? 0 > 0 ? 1 : 0
            return count
        }
        return 1
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        return cell
            ...
            switch indexPath.row {
            case 0 :
                cell.titleLabel.text = "Top Deals".localized
                cell.recommendedProducts = self.homeTopDealsArgoid
            case 1 :
                cell.titleLabel.text = "Recommendations".localized
                cell.recommendedProducts = self.homeRecommendationsArgoid
            case 2 :
                cell.titleLabel.text = "Trending".localized
                cell.recommendedProducts = self.homeTrendingArgoid
            default:
                cell.titleLabel.text = "New Arrivals".localized
                cell.recommendedProducts = self.homeArrivalsArgoid
            }
            return cell
        }else{
            if storistaResult?.type == "highlights"{
                let cell = tableView.dequeueReusableCell(withIdentifier: "StoristaHighlightsTVCell" , for: indexPath) as! StoristaHighlightsTVCell
                cell.parent = self
                let feed = storistaResult?.data
                cell.storistaHihhlights = feed
                cell.setupUI()
                return cell
            }else{
                let cell = tableView.dequeueReusableCell(withIdentifier: "StoristaTVCell" , for: indexPath) as! StoristaTVCell
                let feed = storistaResult?.data?.first?.stories
                cell.adjustLayout(storistaAPI?.getLayout() == "grid" ? .vertical : .horizontal)
                cell.parent = self
                cell.storistaResult = self.storistaResult
                cell.feed = feed
                cell.setupUI()
                cell.headingLabel.text = storistaResult?.title ?? "Feed"
                return cell
            }
        }
        // Insta Condition
...
extension HomeViewController:HomeCollectionGridLayoutUpdate {
    ...
    func updateLayoutAccordingToGrid(viewModel: HomeCollectionGridLayoutViewModel?, collection: UICollectionView?, index: IndexPath, imageSize: CGSize?, reload: Bool) {
        ...
        if layoutHeight[componentName] == nil || reload {
            if viewModel?.header == "1" {
                if viewModel?.item_title == "1" {
                    let size = ((collection?.calculateCellSize(numberOfColumns: 3,of: 75, imagesize: imageSize ?? CGSize(width: 1, height: 1), spacing: 25).height ?? 0 ) + 45) * CGFloat(Int(viewModel?.item_rows ?? "0") ?? 0)
                    layoutHeight[componentName] = size + 40
                }else {
                    let height =  ((collection?.calculateCellSize(numberOfColumns: 3,of: 75, imagesize: imageSize ?? CGSize(width: 1, height: 1), spacing: 20).height ?? 0 )+14) * CGFloat(Int(viewModel?.item_rows ?? "0") ?? 0)
                    layoutHeight[componentName] = height + 40
                }
            }else{
                if viewModel?.item_title == "1" {
                    layoutHeight[componentName] = ((collection?.calculateCellSize(numberOfColumns: 3,of: 75, imagesize: imageSize ?? CGSize(width: 1, height: 1), spacing: 25).height ?? 0 ) + 45) * CGFloat(Int(viewModel?.item_rows ?? "0") ?? 0)
                }else {
                    let height =  ((collection?.calculateCellSize(numberOfColumns: 3,of: 75, imagesize: imageSize ?? CGSize(width: 1, height: 1), spacing: 20).height ?? 0 )+15) * CGFloat(Int(viewModel?.item_rows ?? "0") ?? 0)
                    layoutHeight[componentName] = height
                }
            }
        }
