...
extension HomeViewController
{
    func setupTable()
    {
        ...
        if customAppSettings.sharedInstance.storista{
            let nib = UINib(nibName: "StoristaTVCell", bundle: nil)
            self.tableView.register(nib, forCellReuseIdentifier: "StoristaTVCell")
            
            let nib1 = UINib(nibName: "StoristaHighlightsTVCell", bundle: nil)
            self.tableView.register(nib1, forCellReuseIdentifier: "StoristaHighlightsTVCell")
        ...
        self.tableView?.register(RecommendationsTableCell.self, forCellReuseIdentifier: "RecommendationsTableCell")
                        ...
                        DispatchQueue.global(qos: .userInitiated).async {
                            Client.shared.fetchMultiProducts(ids: graphIds, completion: { [weak self](response, error) in
                                if let response = response {
                                    ...
                                    print("Function: \(#function), line: \(#line), \(String(describing: error?.localizedDescription))")
