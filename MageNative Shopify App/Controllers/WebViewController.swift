...
class WebViewController: BaseViewController, WKNavigationDelegate {
  ...
  override func viewDidLoad() {
    super.viewDidLoad()
    loadPage()
    checkingOrderCompletionStatusViaPolling()
  }
  ...
  override func didReceiveMemoryWarning() {
    ...
    func checkingOrderCompletionStatusViaPolling(){
        Client.shared.fetchCompletedOrder(self.checkoutId, completion: {
            response  in
            if response != nil{
                ...
                print("totalPrice=Called in webview did finish=",self.cartSubTotal)
                
                AppEvents.shared.logEvent(.purchased, valueToSum: self.cartSubTotal, parameters: parameters)
                
                Analytics.logEvent(AnalyticsEventPurchase, parameters: [
                    AnalyticsParameterPrice: CartManager.shared.cartCount])
                let right=UIBarButtonItem(barButtonSystemItem: .done, target: self, action: #selector(self.continuePressed(_:)))
                ...
                self.setOrderRequest(token: self.checkoutId)
